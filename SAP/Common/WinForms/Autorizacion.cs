...
namespace SAP.Common.WinForms
{
    public partial class Autorizacion : Form
    {
        ...
        private readonly LibreriaComun libreriaComun;
        public Autorizacion()
        {
            InitializeComponent();
            libreriaComun = new LibreriaComun();
        }
        ...
        private async void contrasena_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private async Task ValidatePassword()
        {
            try
            {
                var findTypeAccess = await libreriaComun.ValidatePermission(contrasena.Text, SAP.Inicio.interfaz.ToLower());
                if (findTypeAccess == TypeUser.Administrador)
                {
                ...
                else if (findTypeAccess == TypeUser.Operacional)
                {
                    switch (SAP.Inicio.acceso)
                    {
                        case 1:
                ...
                else if (findTypeAccess == TypeUser.Auditor)
                {
                    switch (SAP.Inicio.acceso)
                    {
                        case 2:
                ...
                else if (findTypeAccess == TypeUser.AuditorCanal)
                {
                    switch (SAP.Inicio.acceso)
                    {
                        case 25:
                ...
                else if (findTypeAccess == TypeUser.Supervisor)
                {
                    switch (SAP.Inicio.acceso)
                    {
                        case 13:
