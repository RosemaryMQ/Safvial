...
{
    public partial class AvancesEditados : Form
    {
        int indice = 0;
        ...
        public AvancesEditados()
        {
            InitializeComponent();
            this.Denominaciones();
            Recaudador1.Text = SAP.Tesoreria.Controles.ListaDeclaraciones.nombreusuario;
            indice = 1;
        }
        ...
        {
            if (CodigoT.Text != "" && EsValido(CodigoT.Text))
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    AgregarDeclaracion1(bt1, bt2, bt3, bt4, bt5, bt6, bt7, bt8, bt9, bt10, 0, efectiv, pd, incidenci, Convert.ToInt32(SAP.Tesoreria.Controles.ListaDeclaraciones.ID_Usuario), Convert.ToInt32(SAP.Inicio.ID), Convert.ToInt32(CodigoT.Text), Convert.ToInt32(SAP.Tesoreria.Controles.ListaDeclaraciones.turno), bt11, bt12, bt13, Transferencia, bt14, bt15, bt16, checkBox1.Checked, bt17, bt18, bt19, bt20, bt21, bt22,bt23,bt24,bt25);
                    Avance.Rows.Clear();
                    Denominaciones();
                    CodigoT.Text = "";
        ...
        private void AgregarDeclaracion1(double centimos, double uno, double dos, double cinco, double diez, double veinte, double cincuenta, double cien, double doscientos, double quinientos, double tickets, double efectivo2, double pdv, double incidencia, int id, int id2, int tesorero, int turno, double diezmil1, double veintemil1, double cincuentamil1, double transf, double docientosmil, double quinientosmil, double unmillion, Boolean Buzon, double bd1, double bd5, double bd10, double bd20, double bd50, double bd100, double bd02, double bd05, double bd025)
        {
            string sql = "Insert into CierreBalanceV2 (BilleteS05,BilleteS1,BilleteS2,BilleteS5,BilleteS10,BilleteS20,BilleteS50,BilleteS100,BilleteS200,BilleteS500,Tickets,Efectivo,PDV,Incidencia,ID_Usuario,Fecha,Responsable,TesoreroC,Turno,BilleteS10000,BilleteS20000,BilleteS50000,Transferencia,Eliminado,Buzon,BilleteS200000,BilleteS500000,BilleteS1000000,BilleteBD1,BilleteBD5,BilleteBD10,BilleteBD20,BilleteBD50,BilleteBD100,BilleteBD05,BilleteBD02,BilleteBD025) Values (@centimos,@uno,@dos,@cinco,@diez,@veinte,@cincuenta,@cien,@doscientos,@quinientos,@tickets,@efectivo,@pdv,@incidencia,@id,@fecha,@id2,@tesorero,@turno,@diezmil,@veintemil,@cincuentamil,@transf,0,@buzon,@docientosmil,@quinientosmil, @unmillon, @bd1,@bd5,@bd10,@bd20,@bd50,@bd100,@bd05,@bd02,@bd025)";
            using (SqlConnection cn = new SqlConnection(Inicio.conexion))
            {
                cn.Open();
                SqlCommand cmd = new SqlCommand(sql, cn);
                ...
                cmd.Parameters.AddWithValue("fecha", Convert.ToDateTime(SAP.Tesoreria.Controles.ListaDeclaraciones.fechafinal).AddHours(-1));
